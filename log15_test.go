...
func TestLogfmt(t *testing.T) {
	...
	l.Error("some message", "x", 1, "y", 3.2, "equals", "=", "quote", "\"",
		"nilval", nilVal, "nil", nil, "carriage_return", "bang"+string('\r')+"foo", "tab", "bar	baz", "newline", "foo\nbar")
...
func isTrimPathEnabled() bool {
	_, file, _, ok := runtime.Caller(0)
	if !ok {
		return false
	}
	// If trimpath is used, file paths won't be absolute
	return !filepath.IsAbs(file)
}

// https://github.com/inconshreveable/log15/issues/27
func TestCallerStackHandler(t *testing.T) {
	if isTrimPathEnabled() {
		t.Skip("See log15#27, this test is not compatible with -trimpath")
	}
